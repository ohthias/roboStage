  ...
  return (
    <>
      <header className="space-y-4 bg-gradient-to-r from-primary/5 to-transparent rounded-lg p-6">
        ...
        {/* active filter label */}
        {activeLabel && (
          <p className="text-sm text-neutral text-end opacity-75">
            Filtrando por {activeLabel}
          </p>
        )}
      </header>
      <main className="p-6">
        <div className="border-2 border-dashed border-base-300 rounded-lg h-96 flex items-center justify-center text-base-content/50">
          Área de projetos (em breve)
        </div>
      </main>
    </>
  );
}
function onFilterChange(arg0: {
  category: Category;
  query: string;
  sort: SortOption;
}) {
