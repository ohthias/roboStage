...
export const MissionTable: React.FC<MissionTableProps> = ({
  ...
  const handleCellChange = (missionId: string, colId: string, value: any) => {
    setMissions((prev) =>
      prev
        ? prev.map((m) => (m.id === missionId ? { ...m, [colId]: value } : m))
        : prev
    );
  };
          ...
          <thead className="text-xs text-base-content/70 uppercase bg-base-200 sticky top-0 z-10">
            <tr>
              {columns.map((col) => (
                <th
                  key={col.id}
                  className="px-4 py-3 font-semibold min-w-[150px] group"
                >
                  ...
                  >
                    {col.type === "select" ? (
                      <select
                        ...
                        onChange={(e) =>
                          handleCellChange(mission.id, col.id, e.target.value)
                        }
