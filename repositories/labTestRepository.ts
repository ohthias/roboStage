...
export const labTestRepository = {
  ...
  async insertMissions(testId: string, missions: CreateLabTestDTO["missions"]) {
    ...
    const { error } = await supabase.from("test_missions").insert(
  ...
  async insertVariables(
    ...
    const { error } = await supabase.from("test_variables").insert(
  ...
  async createExecution(
    ...
    const executionNumber = (count ?? 0) + 1;
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        returned_to_base: payload.returnedToBase,
        notes: payload.notes,
        score: payload.score,
        metrics: {
